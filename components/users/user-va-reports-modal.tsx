...
import {
  fetchUserVaReports,
  fetchDownloadUrl,
  FetchVaReportsResult,
} from '@/app/dashboard/users/actions'
import { VaReport } from '@/types/user.type'
import { PaginationMeta } from '@/types/api.type'
import { toast } from 'sonner'

interface UserVAReportsModalProps {
  userId: string
  isOpen: boolean
  onClose: () => void
}

export default function UserVAReportsModal({
  userId,
  isOpen,
  onClose,
}: UserVAReportsModalProps) {
  const [startDate, setStartDate] = useState('')
  const [endDate, setEndDate] = useState('')
  const [page, setPage] = useState(1)
  const [reports, setReports] = useState<VaReport[]>([])
  ...
  const doFetch = (targetPage: number) => {
    ...
    startTransition(async () => {
      const result: FetchVaReportsResult = await fetchUserVaReports(
        userId,
        startDate,
        endDate,
        targetPage,
      ...
      <DialogContent className="max-w-6xl w-full max-h-[90vh] flex flex-col p-0 gap-0 overflow-hidden">
        ...
        <DialogHeader className="px-6 pt-6 pb-4 border-b border-border/50 shrink-0">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Pilih rentang tanggal untuk melihat laporan harian transaksi Virtual Account.
          </p>
        </DialogHeader>
