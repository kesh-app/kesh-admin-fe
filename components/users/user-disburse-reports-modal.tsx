...
interface UserDisburseReportsModalProps {
  userId: string
  isOpen: boolean
  onClose: () => void
}

export default function UserDisburseReportsModal({
  userId,
  isOpen,
  onClose,
}: UserDisburseReportsModalProps) {
  const [startDate, setStartDate] = useState('')
  const [endDate, setEndDate] = useState('')
  const [page, setPage] = useState(1)
  const [reports, setReports] = useState<DisburseReport[]>([])
  ...
  const doFetch = (targetPage: number) => {
    ...
    startTransition(async () => {
      const result: FetchDisburseReportsResult = await fetchUserDisburseReports(
        userId,
        startDate,
        endDate,
        targetPage,
      ...
      <DialogContent className="max-w-6xl w-full max-h-[90vh] flex flex-col p-0 gap-0 overflow-hidden">
        ...
        <DialogHeader className="px-6 pt-6 pb-4 border-b border-border/50 shrink-0">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Pilih rentang tanggal untuk melihat laporan harian disburse.
          </p>
        </DialogHeader>
                      ...
                      reports.map((item, idx) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <TableCell className="text-sm">{item.businessName || '-'}</TableCell>
                            ...
                            <TableCell className="text-right font-mono font-bold text-emerald-500 text-sm">
                              Rp {item.totalCredit.toLocaleString('id-ID')}
                            </TableCell>
